...
const INITIAL_JOBS = [
  {
    ...
    location: "Kochi",
    remote: true,
    skills: ["React", "Supabase", "TypeScript", "Tailwind CSS"],
    time: "2 hours ago",
    description: "We are seeking a senior frontend engineer to build out our collaboration portal. The work involves setting up secure row-level security policies in Supabase, optimizing next-gen dashboard views with React SPA routes, and implementing real-time web socket updates."
  },
  {
    id: "j2",
...
export default function JobsView({ savedJobs, onToggleSaveJob }: JobsViewProps) {
  ...
  return (
    <div className="bg-kasavu-pattern min-h-screen py-10">
      <div className="max-w-7xl mx-auto px-6 md:px-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-10">
          ...
          <button
            ...
            className="mt-5 sm:mt-0 bg-primary hover:bg-primary-hover text-white px-6 py-3.5 rounded-xl font-heading font-bold text-xs sm:text-sm transition-all duration-300 flex items-center space-x-2 shadow-xs hover:-translate-y-0.5 cursor-pointer"
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-7 flex flex-col space-y-6">
            ...
            {/* Filters Box */}
            <div className="bg-white rounded-2xl border border-primary/10 p-5 shadow-3xs flex flex-col space-y-4">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-11 pr-4 py-2.5 rounded-xl border border-primary/10 bg-bg-base/40 focus:bg-white focus:outline-hidden focus:ring-2 focus:ring-primary/20 text-xs sm:text-sm transition-all"
                />
              </div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div className="flex items-center space-x-2">
                  ...
                  <select
                    ...
                    className="bg-bg-base hover:bg-gray-100 text-xs font-semibold rounded-lg border-none px-3 py-1.5 focus:ring-2 focus:ring-primary/20"
                  >
                ...
                <label className="flex items-center space-x-2 cursor-pointer select-none">
                  <input
                    ...
                    className="h-4 w-4 rounded-md border-primary/20 text-primary focus:ring-primary/20"
                  />
                  <span className="text-xs font-heading font-bold text-primary">Remote Only</span>
                </label>
              </div>
            </div>
            ...
            <div className="space-y-4">
              {filteredJobs.length > 0 ? (
                    ...
                    <div
                      ...
                      className={`bg-white border p-6 rounded-2xl cursor-pointer transition-all duration-300 relative flex flex-col justify-between ${
                        isSelected
                          ...
                          : "border-primary/5 hover:border-primary/15"
                      }`}
                    >
                      <div>
                        <div className="flex justify-between items-start">
                          <span className="text-[10px] font-bold text-primary bg-primary/5 px-2 py-0.5 rounded border border-primary/5">
                            {job.type}
                          </span>
                          <button
                            onClick={(e) => {
                        ...
                        <h3 className="font-heading font-bold text-primary text-sm sm:text-base mt-3">
                          {job.title}
                        </h3>
                        <p className="text-[10px] sm:text-xs text-text-muted mt-0.5">{job.company}</p>

                        <div className="flex items-center space-x-4 mt-3 text-xs text-text-muted">
                          <span className="flex items-center">
                            <MapPin className="h-3.5 w-3.5 mr-1 text-primary" /> {job.location} {job.remote && "(Remote)"}
                          </span>
                          <span className="flex items-center">
                            <Clock className="h-3.5 w-3.5 mr-1 text-accent" /> {job.time}
                          </span>
                        </div>
                      </div>

                      <div className="mt-4 pt-4 border-t border-primary/5 flex justify-between items-center">
                        <span className="text-xs sm:text-sm font-heading font-extrabold text-accent">
                          {job.budgetString}
                        </span>
                        ...
                        <div className="flex flex-wrap gap-1">
                          {job.skills.slice(0, 2).map((skill, sIdx) => (
                            <span
                              key={sIdx}
                              className="bg-bg-base text-[10px] text-text-muted px-2 py-0.5 rounded border border-primary/5"
                            >
                              {skill}
                            </span>
                          ))}
                ...
                <div className="py-16 text-center bg-white rounded-2xl border border-dashed border-primary/20">
                  ...
                  <h3 className="font-heading font-bold text-lg text-primary">No jobs found</h3>
          ...
          <div className="lg:col-span-5 sticky top-28 hidden lg:block">
            {selectedJob ? (
              <div className="bg-white border border-primary/10 rounded-2xl p-6 shadow-xs flex flex-col space-y-5">
                <div>
                  <div className="flex justify-between items-start">
                    <span className="text-xs font-bold text-primary bg-primary/5 px-3 py-1 rounded-md border border-primary/5">
                      {selectedJob.type}
                    </span>
                    <span className="text-xs text-text-muted flex items-center">
                      <Clock className="h-3.5 w-3.5 mr-1 text-accent" /> {selectedJob.time}
                    </span>
                  </div>

                  <h2 className="font-heading font-extrabold text-lg sm:text-xl text-primary mt-4 leading-tight">
                    {selectedJob.title}
                  </h2>
                  <p className="text-xs sm:text-sm font-semibold text-accent mt-1">{selectedJob.company}</p>

                  <div className="mt-4 grid grid-cols-2 gap-4 bg-bg-base p-4 rounded-xl border border-primary/5">
                    <div>
                      <span className="text-[9px] text-text-muted uppercase tracking-wider block">Estimated Budget</span>
                      <span className="text-xs sm:text-sm font-heading font-extrabold text-accent">{selectedJob.budgetString}</span>
                    </div>
                    <div>
                      <span className="text-[9px] text-text-muted uppercase tracking-wider block">Location</span>
                      <span className="text-xs sm:text-sm font-heading font-semibold text-primary flex items-center">
                        <MapPin className="h-3.5 w-3.5 mr-1 text-primary animate-pulse" /> {selectedJob.location}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  <h4 className="font-heading font-bold text-xs sm:text-sm text-primary mb-2">Job Description</h4>
                ...
                <div>
                  <h4 className="font-heading font-bold text-xs sm:text-sm text-primary mb-2.5">Skills Required</h4>
                  <div className="flex flex-wrap gap-1.5">
                    {selectedJob.skills.map((skill, sIdx) => (
                      <span
                        key={sIdx}
                        className="bg-bg-base border border-primary/10 text-primary text-xs px-3 py-1 rounded-lg font-medium"
                      >
                        {skill}
                      </span>
                    ))}
                ...
                <div className="pt-4 border-t border-primary/5 flex items-center space-x-4">
                  <a
                    href="mailto:jobs@keralancehub.com?subject=Application"
                    className="flex-1 bg-primary hover:bg-primary-hover text-white text-center py-3 rounded-xl font-heading font-bold text-xs sm:text-sm transition-all shadow-xs"
                  >
                    Apply for this Job
                  </a>
                  <button
                    onClick={() => onToggleSaveJob(selectedJob.id)}
                    className={`px-4 py-3 rounded-xl border transition-all duration-300 flex items-center justify-center cursor-pointer ${
                      savedJobs.includes(selectedJob.id)
                        ? "bg-accent/5 border-accent/20 text-accent"
                        : "bg-white hover:bg-gray-50 border-primary/10 text-text-muted"
                    }`}
                  >
              ...
              <div className="bg-white/50 border border-dashed border-primary/20 rounded-2xl p-12 text-center">
      ...
      {/* Post a Job Dialog Modal */}
      {isFormOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-end bg-black/40 backdrop-blur-xs p-4 sm:p-6">
          <div className="bg-white w-full max-w-xl h-full rounded-2xl shadow-2xl flex flex-col justify-between overflow-hidden border border-primary/10">
            
            {/* Header */}
            <div className="p-6 border-b border-primary/10 flex items-center justify-between bg-bg-base">
              <div className="flex items-center space-x-2">
                <div className="h-8 w-8 rounded-lg bg-primary/10 fill-current text-primary flex items-center justify-center">
                  <Briefcase className="h-4.5 w-4.5" />
                </div>
                <h3 className="font-heading font-bold text-lg text-primary">Post a Freelance Job</h3>
              </div>
              <button
                onClick={() => setIsFormOpen(false)}
                className="p-1.5 rounded-lg hover:bg-gray-100 text-text-muted transition-colors cursor-pointer"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <form onSubmit={handlePostJob} className="flex-1 overflow-y-auto p-6 space-y-5">
              {formSuccess && (
                <div className="bg-emerald-50 border border-emerald-250 rounded-xl p-4 flex items-center space-x-3 text-emerald-800 text-xs font-semibold">
                  ...
                  <span>Job posted successfully! Board updated.</span>
                </div>
              )}
              ...
              <div className="flex items-center space-x-2.5 py-1">
                <input
                  ...
                  className="h-4.5 w-4.5 rounded-md border-primary/20 text-primary"
                />
                <label htmlFor="remoteCheck" className="text-xs font-bold text-primary cursor-pointer select-none">
            ...
            {/* Footer Buttons */}
            <div className="p-6 border-t border-primary/10 bg-bg-base flex items-center justify-end space-x-3">
              <button
                type="button"
                onClick={() => setIsFormOpen(false)}
