...
const PROFILE_DATA: Record<string, any> = {
    ...
    projects: [
      {
        ...
        desc: "Designed and built the full community forum, job board integrations, and live messenger features using Next.js, Supabase, and Tailwind CSS.",
  ...
  f2: {
    ...
    coverGradient: "from-emerald-600 to-yellow-600",
    experience: "5 years",
    projects: [
      {
  ...
  f3: {
    ...
    bgColor: "bg-amber-50",
    coverGradient: "from-amber-600 to-teal-800",
    experience: "6 years",
    projects: [
      {
...
export default function ProfilePage({ params }: ProfilePageProps) {
  ...
  const freelancer = PROFILE_DATA[id] || PROFILE_DATA.f1; // default fallback
  ...
  return (
    <div className="bg-mesh-gradient min-h-screen pb-20">
      
      {/* 1. Large Cover Header */}
      <div className={`h-64 sm:h-80 w-full bg-gradient-to-r ${freelancer.coverGradient} relative`}>
        {/* Wave pattern overlay */}
        <div className="absolute inset-0 bg-mesh-gradient-reverse opacity-20 mix-blend-overlay" />
        
        {/* Back Link */}
        <div className="max-w-7xl mx-auto px-6 md:px-8 pt-6 relative z-10">
          <Link
            href="/directory"
            className="inline-flex items-center space-x-1.5 text-white/80 hover:text-white bg-black/20 backdrop-blur-xs px-3.5 py-1.5 rounded-xl text-xs font-heading font-semibold transition-all"
          >
      ...
      {/* 2. Main Profile Layout container */}
      <div className="max-w-7xl mx-auto px-6 md:px-8 -mt-24 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          
          {/* Left Column: Avatar & Basic Details Card */}
          <div className="lg:col-span-4 bg-white border border-gray-200/80 rounded-2xl p-6 shadow-sm flex flex-col items-center text-center">
            
            {/* Avatar */}
            <div className={`h-32 w-32 rounded-2xl ${freelancer.bgColor} border-4 border-white shadow-md flex items-center justify-center text-primary font-heading font-extrabold text-3xl mb-4`}>
              {freelancer.avatar}
            </div>

            <h1 className="font-heading font-extrabold text-2xl text-text-main tracking-tight">
              {freelancer.name}
            </h1>
            <p className="text-sm text-text-muted mt-0.5">{freelancer.role}</p>

            <div className="flex items-center space-x-1 mt-3">
              <Star className="h-4.5 w-4.5 text-accent fill-accent" />
              <span className="text-sm font-bold text-text-main">{freelancer.rating}</span>
              <span className="text-xs text-text-muted">({freelancer.reviewsCount} reviews)</span>
            </div>
            ...
            {/* Status Badge */}
            <div className="w-full mt-6 pt-4 border-t border-gray-100 flex justify-between items-center text-left">
              <span className="text-xs text-text-muted font-heading font-semibold">Availability</span>
              {freelancer.available ? (
                <span className="bg-emerald-50 text-emerald-700 text-xs font-semibold px-3 py-1 rounded-full border border-emerald-100 flex items-center">
            ...
            <div className="w-full mt-4 flex justify-between items-center text-left">
              ...
              <span className="text-sm font-semibold text-text-main">{freelancer.experience}</span>
            </div>

            {/* CTA Contact Button */}
            <button
              onClick={() => setIsContactOpen(true)}
              className="w-full mt-8 bg-primary hover:bg-primary-hover text-white py-3.5 rounded-xl font-heading font-bold text-sm transition-all duration-300 flex items-center justify-center space-x-2 shadow-md shadow-primary/10"
            >
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            <div className="bg-white border border-gray-200/80 rounded-2xl p-6 sm:p-8 shadow-xs">
              <h2 className="font-heading font-bold text-lg text-text-main mb-3">About Me</h2>
              <p className="text-sm sm:text-base text-text-muted leading-relaxed font-sans">
                {freelancer.bio}
              </p>

              <h3 className="font-heading font-bold text-sm text-text-main mt-6 mb-3">Core Expertise</h3>
              <div className="flex flex-wrap gap-1.5">
                {freelancer.skills.map((skill: string, sIdx: number) => (
                  <span
                    key={sIdx}
                    className="bg-gray-50 border border-gray-200/60 text-text-main text-xs px-3 py-1.5 rounded-lg"
                  >
                    {skill}
                  </span>
                ))}
            ...
            <div className="flex border-b border-gray-200 space-x-6">
              <button
                ...
                className={`pb-3 text-sm font-heading font-bold transition-all relative ${
                  activeTab === "portfolio"
                    ? "text-primary"
              ...
              {activeTab === "portfolio" ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {freelancer.projects.map((proj: any, pIdx: number) => (
                    <div
                      key={pIdx}
                      className="bg-white border border-gray-200/80 rounded-2xl p-6 shadow-2xs hover:shadow-xs hover:border-gray-300 transition-all flex flex-col justify-between h-52"
                    >
                      <div>
                ...
                <div className="space-y-4">
                  {freelancer.reviews.map((rev: any, rIdx: number) => (
                    <div
                      key={rIdx}
                      className="bg-white border border-gray-200/80 rounded-2xl p-6 shadow-2xs space-y-4"
                    >
                      <div className="flex justify-between items-start">
                        <div>
                          <h4 className="font-heading font-bold text-sm text-text-main">{rev.author}</h4>
                          <p className="text-[11px] text-text-muted">{rev.company}</p>
                        </div>
      ...
      {isContactOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 backdrop-blur-xs p-4">
          <div className="bg-white w-full max-w-lg rounded-2xl shadow-xl overflow-hidden border border-gray-200 animate-scaleUp">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50">
              <h3 className="font-heading font-bold text-lg text-text-main flex items-center">
            ...
            <form onSubmit={handleSendMessage} className="p-6 space-y-4">
              ...
              <div className="pt-4 flex items-center justify-end space-x-3 border-t border-gray-100">
                ...
                <button
                  ...
                  className="bg-primary hover:bg-primary-hover text-white px-6 py-2.5 rounded-xl text-xs font-heading font-bold transition-all flex items-center space-x-1.5 shadow-sm"
                >
