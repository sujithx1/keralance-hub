import { Sparkles, Heart, Award, ShieldCheck, Mail, ArrowUpRight } from "lucide-react";
    ...
    <div className="bg-mesh-gradient min-h-screen py-16">
      <div className="max-w-4xl mx-auto px-6 md:px-8 space-y-16">
        ...
        <div className="text-left space-y-4">
          <div className="inline-flex items-center space-x-2 bg-soft-accent border border-emerald-200/50 px-3 py-1 rounded-full text-xs font-semibold text-primary">
        ...
        <div className="space-y-8">
          <h2 className="font-heading font-extrabold text-2xl text-text-main tracking-tight border-b border-gray-200 pb-3">
            Our Core Values
          </h2>
            ...
            {VALUES.map((val, idx) => {
              ...
              return (
                <div key={idx} className="space-y-3">
                  <div className="h-10 w-10 rounded-xl bg-soft-accent flex items-center justify-center text-primary">
                    <Icon className="h-5 w-5" />
                  </div>
                  <h3 className="font-heading font-bold text-base text-text-main">{val.title}</h3>
                  <p className="text-xs sm:text-sm text-text-muted leading-relaxed font-sans">{val.desc}</p>
                </div>
              );
            })}
          </div>
        ...
        {/* Vision statement */}
        <div className="bg-white border border-gray-200/80 rounded-2xl p-8 shadow-2xs space-y-4 relative overflow-hidden">
          <div className="absolute inset-0 bg-mesh-gradient-reverse opacity-40 pointer-events-none" />
          <h2 className="font-heading font-extrabold text-xl text-text-main relative z-10">Our Mission</h2>
          <p className="text-sm sm:text-base text-text-muted leading-relaxed font-sans relative z-10">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between bg-gradient-to-tr from-primary to-secondary p-8 rounded-2xl text-white shadow-md">
          <div className="space-y-2 text-center sm:text-left mb-6 sm:mb-0">
            <h3 className="font-heading font-bold text-lg">Have questions or want to partner?</h3>
            <p className="text-xs text-emerald-100">We're always open to sponsorships, partnerships, and collaborations.</p>
          </div>
          <Link
            href="/contact"
            className="bg-white hover:bg-gray-100 text-primary px-6 py-3 rounded-xl font-heading font-bold text-xs transition-all flex items-center space-x-1.5 shadow-sm"
          >
